...
import { Search, Filter, Grid, List, TrendingUp, Star, Sparkles } from 'lucide-react';
...
export default function NewArrivalsPage() {
  ...
  // Fetch all products including trending, best sellers, and Supabase products
  const fetchProducts = async () => {
    try {
      setLoading(true);
      
      // Fetch from multiple sources
      const [supabaseProducts, cjProducts, localProducts] = await Promise.all([
        fetch('/api/products?limit=5&source=supabase').then(res => res.json()),
      ...
      // Combine all products and mark their source
      const allProducts = [
        ...(supabaseProducts.products || []).map((p: Product) => ({ ...p, source: 'supabase' as const })),
        ...(cjProducts.products || []).map((p: Product) => ({ ...p, source: 'cj' as const })),
        ...(localProducts.products || []).map((p: Product) => ({ ...p, source: 'local' as const }))
      ];

      // Mark trending and bestseller products
      const processedProducts = allProducts.map((product, index) => ({
        ...product,
        trending: index % 4 === 0, // Mark every 4th product as trending
        bestseller: index % 6 === 0, // Mark every 6th product as bestseller
        new_arrival: index % 3 === 0, // Mark every 3rd product as new arrival
  ...
  const getFilteredProducts = () => {
    ...
    switch (activeTab) {
      ...
      case 'bestsellers':
        ...
        break;
      case 'new':
        filtered = products.filter(p => p.new_arrival);
        break;
      case 'featured':
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            <p className="text-gray-600 text-lg">
              Discover the latest products including trending items, best sellers, and everything from our Supabase database
            </p>
          </motion.div>
